...
该命令用于一次性执行仓库内的核心自动化测试，主要覆盖：

1. 白盒测试：验证订单核验、开始服务、完成服务、评价、售后、通知等核心云函数的状态流转、权限校验和异常返回。
2. 接口契约测试：逐个扫描业务云函数，确认返回结构符合统一的 `code / message / data / requestId` 响应约定。
3. 核心链路联调：模拟预约订单从到店核验、开始服务、完成服务到评价创建的主流程，确认关键业务闭环可运行。
4. 数据一致性与性能基础测试：检查队列提示、订单状态更新、评价聚合等数据处理逻辑，并对典型列表/队列计算做基础耗时验证。

测试脚本使用本地 mock 的 uniCloud 数据库与上下文，不会连接作者个人服务空间，也不会发送真实短信或调用真实 AI 服务。测试报告会生成到 `docs/接口联调测试结果.txt` 和 `docs/数据与性能基础测试结果.txt`。
