...
## 近期更新（2026-02）
- 资料保存实时同步：
  - 用户端、理发师端、管理员端主界面均已接入 `user-profile-updated` 事件，保存账号名/头像后无需手动刷新即可更新页面展示。
  - 保存后仍会回源 `me()` 做二次校准，确保本地状态与数据库最终一致。
- 默认头像展示规则统一：
  - 无头像时使用“首字符占位头像”；
  - 所有主界面与门店详情页默认头像统一为黑色背景 + 白色文字；
  - 图片加载失败自动回退到默认占位头像。
- 首页问候语改为实时时段文案：
  - `17:00-03:59` 晚上好，`04:00-08:59` 早上好，`09:00-11:59` 上午好，`12:00-12:59` 中午好，`13:00-16:59` 下午好。
- 用户首页展示名规则：
  - 当前按 `username > name > nickname` 取值，默认头像首字符与问候语用户名保持一致。
